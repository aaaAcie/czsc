...
def main():
    out_path = Path("tests/moore/baselines/micro_engine_refactor_v2.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {"version": "v2", "scenarios": []}
    for sc in SCENARIOS:
        bars = research.get_raw_bars_origin(sc["symbol"], sdt=sc["sdt"], edt=sc["edt"])
        if not bars:
            payload["scenarios"].append({"name": sc["name"], "symbol": sc["symbol"], "error": "no_bars"})
            continue
