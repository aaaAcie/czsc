...
当前实现：
  ...
  - weekly_segment_analyzer：周线级别线段分析（消费日线层最终结果）
...
class MooreCZSC:
    ...
    def __init__(
        ...
        # --- 周线级别线段 ---
        self.weekly_segment_analyzer = WeeklySegmentAnalyzer(
            self.daily_segment_analyzer.daily_segments,
            daily_centers=self.daily_segment_analyzer.daily_centers,
            daily_refined_segments=self.daily_segment_analyzer.refined_segments
            + self.daily_segment_analyzer.pending_refined_segments,
            ma170=self.daily_segment_analyzer.state.ma170,
        )
    ...
    def update(self, bar: RawBar):
        ...
        self.weekly_segment_analyzer.update(
            self.daily_segment_analyzer.daily_segments,
            daily_centers=self.daily_segment_analyzer.daily_centers,
            daily_refined_segments=self.daily_segment_analyzer.refined_segments
            + self.daily_segment_analyzer.pending_refined_segments,
    ...
    def weekly_segments(self) -> List[WeeklySegment]:
        return self.weekly_segment_analyzer.weekly_segments

    @property
    def weekly_pending_segments(self) -> List[WeeklySegment]:
        return self.weekly_segment_analyzer.weekly_pending_segments

    @property
    def weekly_non_same_segments(self) -> List[WeeklySegment]:
        return self.weekly_segment_analyzer.weekly_non_same_segments

    @property
    def weekly_centers(self) -> List[WeeklySegmentCenter]:
        return self.weekly_segment_analyzer.weekly_centers

    @property
    def weekly_pending_centers(self) -> List[WeeklySegmentCenter]:
        return self.weekly_segment_analyzer.weekly_pending_centers

    @property
    def weekly_center_source_segments(self) -> List[DailySegment]:
        return self.weekly_segment_analyzer.weekly_center_source_segments
