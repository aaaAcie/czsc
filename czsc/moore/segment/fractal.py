...
class FractalEngine:
    ...
    def _process_confirmed_trigger(self, turning_k_bar: RawBar, turning_k_index: int, new_mark: Mark):
        ...
        if s.candidate_tk:
            if self._can_refresh_candidate(extreme_bar, extreme_k_index, new_price, new_mark):
    ...
    def _can_refresh_candidate(self, new_extreme_bar: RawBar, new_extreme_idx: int, new_price: float, new_mark: Mark) -> bool:
        """判断新找到的极值K能否刷新当前候选顶底（candidate_tk）"""
        s = self.s
        if not s.candidate_tk:
            return True

        old_tk    = s.candidate_tk
        old_bar   = old_tk.raw_bar
        new_ma5   = new_extreme_bar.cache.get('ma5', 0.0)

        # 【MA5 能量区间对比法则】：
        # 能量刷新不应只对比旧点，必须对比整段线段区域内的 MA5 极值，以消除均线滞后的误导。
        ref_idx = s.turning_ks[-1].k_index + 1 if s.turning_ks else 0

        # 获取此前的线段区域内所有的 MA5
        # 注意：这里取到 new_extreme_bar 之前的所有历史，寻找这段趋势已创下的动能极点
        past_bars = s.bars_raw[ref_idx : new_extreme_idx]
        if not past_bars:
            ref_ma5 = old_bar.cache.get('ma5', 0.0)
        else:
            if new_mark == Mark.G:
                ref_ma5 = max(b.cache.get('ma5', 0.0) for b in past_bars)
            else:
                ref_ma5 = min(b.cache.get('ma5', 1e9) for b in past_bars)
        
        # --- 法则一 (Growth)：价格新极值 + 实体穿透 ---
        if new_mark == Mark.G:
            new_top = max(new_extreme_bar.open, new_extreme_bar.close)
            old_bottom = min(old_bar.open, old_bar.close)
            cond_B = new_price > old_tk.price and new_top > old_bottom
        else:
            new_bottom = min(new_extreme_bar.open, new_extreme_bar.close)
            old_top = max(old_bar.open, old_bar.close)
            cond_B = new_price < old_tk.price and new_bottom < old_top

        # --- 法则二A (Energy)：MA5 能量覆盖（超越整段极值） ---
        cond_A = (new_ma5 > ref_ma5) if new_mark == Mark.G else (new_ma5 < ref_ma5)
    ...
    def _validate_four_rules(self, tk: TurningK) -> tuple:
        ...
        if 0 < idx < len(s.bars_raw) - 1:
            ...
            return False, False # 右侧K线未完成或不存在
        ...
        active = self.center.get_active_center()
        if active:
            all_avail_centers.append(active)

        for c in all_avail_centers:
            # 只要中枢的生命周期归属于本段，则构成本段的基础结构。扫描范围：[ref_tk.k_index, tk.k_index]
            if ref_tk.k_index <= c.start_k_index <= tk.k_index and not getattr(c, 'is_ghost', False):
                is_perfect = True
                break
